        ...
        <div>
            <Slider {...settings}>
                {data?.slice(0,7)?.map((item) => (
                    <div key={item.id} className='bg-gradient-to-r from-[#0f0c29] via-[#302b63] to-[#24243e] -z-10'>
                        <div className='flex flex-col md:flex-row gap-10 justify-center h-[600px] my-20 md:my-0 items-center px-4'>
                            ...
                            <Link to={`/products/${item.id}`}>
                                <img src={item.image} alt={item.title} className='rounded-full w-[550px] hover:scale-105 transition-all shadow-2xl shadow-red-400 cursor-pointer'/>
                            </Link>
                        </div>
                    </div>
                ))}
